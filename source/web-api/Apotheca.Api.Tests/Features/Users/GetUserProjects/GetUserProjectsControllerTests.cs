...
namespace Apotheca.Api.Tests.Features.Users.GetUserProjects;
...
public class GetUserProjectsControllerTests
{
    ...
    public void SetUp()
    {
        ...
        _repository.GetProjectsByUidAsync(_dbContext, Arg.Any<string>(), Arg.Any<string>())
            .Returns(Task.FromResult(Enumerable.Empty<ProjectDbEntity>()));
        _repository.GetProjectStatsAsync(_dbContext, Arg.Any<string>(), Arg.Any<string>())
    ...
    public async Task GetUserProjects_Returns401_WhenIdentityFails()
    {
        ...
        var result = await _controller.GetUserProjects(WorkspaceId, CancellationToken.None);
    ...
    public async Task GetUserProjects_Returns401_WithErrorMessage_WhenIdentityFails()
    {
        ...
        var result = (UnauthorizedObjectResult)await _controller.GetUserProjects(WorkspaceId, CancellationToken.None);
    ...
    [Test]
    public async Task GetUserProjects_ReturnsBadRequest_WhenWorkspaceIdIsMissing()
    {
    ...
    public async Task GetUserProjects_ReturnsEmptyList_WhenUserHasNoProjects()
    {
        ...
        var result   = (OkObjectResult)await _controller.GetUserProjects(WorkspaceId, CancellationToken.None);
    ...
    public async Task GetUserProjects_QueriesProjectsWithFirebaseUid()
    {
        ...
        await _controller.GetUserProjects(WorkspaceId, CancellationToken.None);

        await _repository.Received(1).GetProjectsByUidAsync(_dbContext, "firebase-uid", WorkspaceId);
    }
    ...
    public async Task GetUserProjects_QueriesStatsWithFirebaseUid()
    {
        ...
        await _repository.Received(1).GetProjectStatsAsync(_dbContext, "firebase-uid", WorkspaceId);
    }
    ...
    public async Task GetUserProjects_MergesStatsIntoResponse()
    {
        ...
        _repository.GetProjectsByUidAsync(_dbContext, Arg.Any<string>(), Arg.Any<string>())
            .Returns(Task.FromResult<IEnumerable<ProjectDbEntity>>(new[]
            {
                new ProjectDbEntity { Id = "p1", Name = "Alpha", ProjectRole = "ADMIN", CreatedAt = DateTimeOffset.UtcNow },
            }));
        _repository.GetProjectStatsAsync(_dbContext, Arg.Any<string>(), Arg.Any<string>())
        ...
        Assert.That(project.ProjectRole,   Is.EqualTo("ADMIN"));
