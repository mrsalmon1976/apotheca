        ...
        <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="dialog-title">
          ...
          <div class="dialog-header">
            <div class="dialog-header-left">
              ...
              <h2 id="dialog-title" class="dialog-title">New Workspace</h2>
            </div>
...
async function save() {
  ...
  saving.value = true
  try {
    const created = await createWorkspace(form.name.trim())
    if (created) {
      emit('saved')
      emit('close')
      router.push('/dashboard')
    } else {
      saveError.value = 'Could not create the workspace. Please try again.'
    }
  } finally {
    saving.value = false
  }
