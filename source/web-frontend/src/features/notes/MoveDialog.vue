          ...
          <div class="dialog-header">
            <div class="dialog-header-left">
              ...
              <h2 id="dialog-title" class="dialog-title">
                Move {{ item?.isFolder ? 'Folder' : 'Note' }}
              </h2>
            </div>
...
const { getNotes, moveNote } = useNoteFolders()
...
async function loadFolders(parentId) {
  ...
  try {
    const response = await getNotes(props.projectId, parentId)
    if (response.ok) {
      const items = await response.json()
      folders.value = items.filter(n => n.isFolder)
    } else {
...
async function confirmMove() {
  ...
  try {
    const response = await moveNote(props.projectId, props.item.id, browseFolderId.value)
