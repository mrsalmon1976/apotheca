...
const { createFolder, searchLabels } = useDocumentFolders()
...
function commitLabel(text) {
  ...
  if (trimmed && !selectedLabels.value.includes(trimmed)) selectedLabels.value.push(trimmed)
  labelQuery.value       = ''
  suggestions.value      = []
  highlightedIndex.value = -1
}
...
function removeLabel(index) { selectedLabels.value.splice(index, 1) }
...
function validate() {
  ...
  if (!trimmed) { fieldError.value = 'Folder name is required.'; return null }
  if (trimmed.length < MIN_LENGTH) { fieldError.value = `Folder name must be at least ${MIN_LENGTH} characters.`; return null }
  fieldError.value = null
  return trimmed
}
...
async function save() {
  ...
  if (!title) { nameInput.value?.focus(); return }
...
.label-input-box.focused { border-color: var(--color-purple); box-shadow: 0 0 0 3px rgba(168, 85, 247, 0.15); }
